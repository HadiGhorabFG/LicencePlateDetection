...
Mat Avg(Mat Grey, int neighborSize)
{
	int neighbirSize = neighborSize * ((float)Grey.rows / 600.0f);
...
Mat Edge(Mat Grey, int threshold)
{
	int th = threshold / ((float)Grey.rows / 600.0f);
...
float BlackToWhiteRatio(Mat img)
{
...
{
	Mat img;
	img = imread("C:\\My Files\\School\\CarPics\\1 (" + to_string(i) + ").jpg");
	...
	for (int i = 0; i < contours1.size(); i++)
	{
		...
		Mat rectCut = DilatedImg2(rect);

		if (((float)rect.width / (float)dst.rows) < 0.1f || ((float)rect.width / (float)dst.rows) > 0.25f
			|| ((float)rect.width / (float)dst.rows) > 0.3f
			|| rect.x < 0.1 * GreyImg.cols || rect.x > 0.95 * GreyImg.cols || rect.y < 0.05 * GreyImg.rows
			|| rect.y > 0.9 * GreyImg.rows || ratio < 1.5 || BlackToWhiteRatio(rectCut) > 0.5f)
		{
	...
	//imshow("Filtered Image " + to_string(i), DilatedImgCpy);
